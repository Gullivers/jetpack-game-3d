...
public class GG_BotJetpackMovement : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        else
        {
            ...
            if (DummyFalling)
            {
                DummyJetPackOn = true;
                rb.useGravity = true;
                transform.DOLocalRotate(new Vector3(0, 0, 0), .5f).SetId("FallingAngleBot");
                DummyFalling = false;
    ...
    {   //Y Z pos 
        ...
        transform.DOMoveY(botTrajectory.hitPoint.y, SoftLaunchDuration).SetEase(LaunchEase).SetId("Softlaunch");
        //Z pos
